...
class BannerController extends Controller {
    ...
    public function actionUpdate($id) {
        ...
        if (isset($_POST['Banner'])) {
            $model->attributes = $_POST['Banner'];
            $model->fileAd = CUploadedFile::getInstance($model, 'fileAd');
            if ($model->fileAd) {
                if ($model->validate(array('fileAd'))) {
                    $fileName = $this->getAndSaveUploadedFile($model);
                    if ($fileName) {
                        if ($model->ad_path && file_exists($model->ad_path)) {
                            unlink($model->ad_path);
                        }
                        $model->ad_path = $fileName;
                    }
                }
    ...
    private function getAndSaveUploadedFile($model) {
        ...
        $fileName = null;
        if ($file) {
            $fileName = Yii::app()->params['banner'] . '/' . $file->getName();
            if (file_exists($fileName)) {
                $fileName = Yii::app()->params['banner'] . '/' . time() . '_' . $file->getName();
            }
